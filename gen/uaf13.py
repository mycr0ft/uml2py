...
272 stereotypes, 118 tagged values, 269 constraints
...
_URI = 'http://www.omg.org/spec/UAF/20241101/UAF'
...
class MissionKind(_enum.Enum):
    NationalPolicy = "NationalPolicy"
    TheaterStrategy = "TheaterStrategy"
    Campaign = "Campaign"
    MajorOperation = "MajorOperation"
    Battle = "Battle"
    Engagement = "Engagement"
    SmallUnitAndCrewAction = "SmallUnitAndCrewAction"
    Other = "Other"
...
class Activity(U.Activity, AffectableElement, PropertySet):
...
class ActualMission(ActualStrategicPhase, U.InstanceSpecification):
    """A particular mission to accomplish assigned mission objectives"""
    _STEREO = "UAF::Processes::ActualMission"
    _BASE_METACLASSES = ("InstanceSpecification",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("ActualMission.classifier",
         "Value for the classifier metaproperty must be stereotyped by «Mission» or its specializa"
         "tions."
        ),
    )

class ActualMissionPhase(ActualMission, U.InstanceSpecification):
    """A particular phase in the accomplishment of an overall actual mission"""
    _STEREO = "UAF::Processes::ActualMissionPhase"
    _BASE_METACLASSES = ("InstanceSpecification",)
    _ABSTRACT = False

class ActualMissionScenario(ActualCondition, U.InstanceSpecification):
    """A particular Mission Scenario within which an Actual Mission is to be accomplished"""
    _STEREO = "UAF::Parameters::ActualMissionScenario"
    _BASE_METACLASSES = ("InstanceSpecification",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("ActualMissionScenario.classifier",
         "Value for the classifier metaproperty has to be stereotyped «MissionScenario» or its spe"
         "cializations."
        ),
    )

class ActualMissionVignette(ActualCondition, U.InstanceSpecification):
    """A particular Mission Vignette within which an Actual Mission is to be accomplished"""
    _STEREO = "UAF::Parameters::ActualMissionVignette"
    _BASE_METACLASSES = ("InstanceSpecification",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("ActualMissionVignette.classifier",
         "Value for the classifier metaproperty has to be stereotyped «MissionVignette» or its spe"
         "cializations."
        ),
    )
...
class ActualOrganizationalResource(ActualResource, U.InstanceSpecification, Stakeholder):
    ...
    _ABSTRACT = True
    _DECL = {
    # A relationship between an actual organization and the standards created by that organization
    'createdStandards': _Ref('createdStandards', 'Standard', multi=True, lo=0, hi='*'),
    # A relationship between an actual organization and the standards published by that organization
    'publishedStandards': _Ref('publishedStandards', 'Standard', multi=True, lo=0, hi='*'),
...
class Asset(sysml.Block, U.Class, ConceptItem, PropertySet, SubjectOfSecurityConstraint, AffectableElement):
...
class OpposableElement(UAFElement, U.Element):
    """A type of element that is categorized to allow for it to have an Opposes relationship to some other opposable element"""
    _STEREO = "UAF::Taxonomy::OpposableElement"
    _BASE_METACLASSES = ("Element",)
    _ABSTRACT = True
    _DECL = {
    # Captures the kind of designation.
    'designation': _Ref('designation', 'DesignationKind'),
    }

class ResourcePerformer(ResourceAsset, VersionedElement, SubjectOfForecast, U.Class, ResourceExchangeItem, OperationalExchangeItem, ServiceExchangeItem, StrategicExchangeItem, SubjectOfResourceConstraint, CapableElement, Desirer, OpposableElement):
...
class MotivationalElement(sysml.Block, U.Class, PropertySet, PhaseableElement):
...
class ConflictsWith(U.Abstraction, MeasurableElement, sysml.Problem):
    """An annotation that indicates UAF elements acting against or in opposition to each other. This means that there are elements in the model that disagree about something important or are incompatible with each other in some way"""
    _STEREO = "UAF::Information::ConflictsWith"
    _BASE_METACLASSES = ("Abstraction",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("ConflictsWith.annotatedElement",
         "Value for the annotatedElement metaproperty must be stereotyped by the specialization of"
         " «UAFElement»."
        ),
    )
...
class Defines(sysml.Allocate, MeasurableElement, U.Element):
    """A tuple that links the StrategicPhase and the Activity that implements it"""
    _STEREO = "UAF::Processes::Defines"
    _BASE_METACLASSES = ("Element",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("Defines.client",
         "Value for the client metaproperty must be stereotyped by any of specializations of «Acti"
         "vity»."
        ),
        ("Defines.supplier",
         "Value for the supplier metaproperty must be stereotyped «StrategicPhase» or its speciali"
         "zations."
        ),
    )
...
class DesignationKind(sysml.ValueType, MeasurableElement, U.Element):
    """An enumerated type that defines the kind of structural element with respect to purpose, intent and capabilities of that element"""
    _STEREO = "UAF::Taxonomy::DesignationKind"
    _BASE_METACLASSES = ("Element",)
    _ABSTRACT = False
...
class Standard(SubjectOfForecast, sysml.Block, U.Class, PropertySet):
    ...
    _ABSTRACT = False
    _DECL = {
    # The organization or creator of the standard or doctrine
    'createdBy': _Ref('createdBy', 'ActualOrganizationalResource', multi=True, lo=0, hi='*'),
    # The date when this version of the Standard was published.
    'mandatedDate': _Ref('mandatedDate', 'ISO8601DateTime'),
    # An entity responsible for making the resource available
    'publishedBy': _Ref('publishedBy', 'ActualOrganizationalResource', multi=True, lo=0, hi='*'),
    # Relates a Standard to the ActualOrganization that ratified the Standard.
    'ratifiedBy': _Ref('ratifiedBy', 'ActualOrganizationalResource', multi=True, lo=0, hi='*'),
    # The date when this version of the Standard was retired.
    'retiredDate': _Ref('retiredDate', 'ISO8601DateTime'),
    }

class Doctrine(Standard, sysml.Requirement, U.Class):
    """A formal expression of military knowledge and thought, accepted as being relevant at a given time, which covers the nature of conflict, the preparation of the military forces for conflict, and the method of engaging in conflict to achieve success. it is descriptive rather than prescriptive, requiring judgment in application."""
    _STEREO = "UAF::Taxonomy::Doctrine"
    _BASE_METACLASSES = ("Class",)
    _ABSTRACT = False
...
class EnvironmentProperty(U.Property, MeasurableElement):
    ...
    CONSTRAINTS = (
        ("EnvironmentalProperty.class",
         "Value for the class metaproperty must be stereotyped «PropertySet» or its specialization"
         "s."
        ),
...
class Implements(sysml.Allocate, U.Abstraction, MeasurableElement):
    ...
    CONSTRAINTS = (
        ("Implements.client",
         ...
         "he client metaproperty must be stereotyped by any of specializations of «Architecture», "
         "«OperationalActivity», or «Function». k. «Service» or its specializations, values for th"
         "e supplier metaproperty must be stereotyped «ResourceService» or its specializations. l."
         " «StrategicInformation» or its specializations, values for the client metaproperty must "
         "be stereotyped «OperationalInformation» or its specializations."
        ),
        ("Implements.supplier",
         ...
         "he supplier metaproperty must be stereotyped «OperationalActivity», «ServiceFunction», «"
         "ActualStrategicPhase», or their specializations, c. «ResourceInterface» or its specializ"
         "ations, values for the supplier metaproperty must be stereotyped «ServiceInterface», «Op"
         "erationalInterface», or their specializations, d. «ResourceConnector» or its specializat"
         "ions, values for the supplier metaproperty must be stereotyped «OperationalConnector», «"
         "ResourceConnector» or their specializations, e. «ResourceExchange» or its specialization"
         "s, values for the supplier metaproperty must be stereotyped «OperationalExchange» or its"
         " specializations, f. «ResourceRole» or its specializations, values for the supplier meta"
         "property must be stereotyped «OperationalRole» or its specializations, g. «OperationalAc"
         "tivity» or its specializations, values for the supplier metaproperty must be stereotyped"
         " «ActualStrategicPhase» or its specializations, h. «ResourceInformation» or its speciali"
         "zations, values for the supplier metaproperty must be stereotyped «OperationalInformatio"
         "n» or its specializations. i. by any of specializations of «Architecture», values for th"
         "e supplier metaproperty must be stereotyped by any of specializations of «ActualStrategi"
         "cPhase». j. «ResourceService» or its specializations, values for the supplier metaproper"
         "ty must be stereotyped «Service» or its specializations. k. «OperationalInformation» or "
         "its specializations, values for the supplier metaproperty must be stereotyped «Strategic"
         "Information» or its specializations. l. «ServiceInterface» or its specializations, value"
         "s for the supplier metaproperty must be stereotyped «OperationalInterface» or its specia"
         "lizations."
        ),
    )
...
class OperationalPerformer(OperationalAgent, U.Class, OpposableElement):
...
class LocationHolder(UAFElement, U.Element):
    ...
    _ABSTRACT = True
    _DECL = {
    # Relates a LocationHolder (i.e. OperationalRole, ResourceRole) to its ActualLocation.
    'physicalLocation': _Ref('physicalLocation', 'ActualLocation', multi=True, lo=0, hi='*'),
    # Relates a LocationHolder (i.e. OperationalRole, ResourceRole) to the Condition in which it is re
    # quired to perform/be used.
    'requiredEnvironment': _Ref('requiredEnvironment', 'Condition', multi=True, lo=0, hi='*'),
    }
...
class Mission(StrategicPhase, U.Class):
    """The task, together with the purpose, that clearly indicates the action to be taken and the reasoning behind the mission. (JP 1-02)"""
    _STEREO = "UAF::Taxonomy::Mission"
    _BASE_METACLASSES = ("Class",)
    _ABSTRACT = False
    _DECL = {
    # Identification of a special kind of Mission that is different from one of the predefined enumera
    # ted kinds.
    'customKind': _Ref('customKind', str),
    # Selection of the enumerated kind for this element.
    'kind': _Ref('kind', MissionKind),
    }

class MissionEngineeringThread(Function, U.Activity):
    """A sequence of end-to-end mission-related functions, actions, and events presented as a series of steps that implements a mission thread."""
    _STEREO = "UAF::Processes::MissionEngineeringThread"
    _BASE_METACLASSES = ("Activity",)
    _ABSTRACT = False

class MissionScenario(Condition, U.DataType):
    """Description of the geographical location and time frame of the overall conflict. A scenario includes information such as threat and friendly politico-military contexts and backgrounds, assumptions, constraints, limitations, strategic objectives, and other planning considerations. (OUSD(R&E))"""
    _STEREO = "UAF::Parameters::MissionScenario"
    _BASE_METACLASSES = ("DataType",)
    _ABSTRACT = False
...
class MissionThread(OperationalActivity, U.Activity):
    """A sequence of end-to-end mission tasks, activities, and events presented as a series of steps to achieve a mission. (OUSD(R&E))"""
    _STEREO = "UAF::Processes::MissionThread"
    _BASE_METACLASSES = ("Activity",)
    _ABSTRACT = False

class MissionTask(MissionThread, U.Activity):
    """A clearly defined activity specifically to be assigned to system, person, or organization that must be completed"""
    _STEREO = "UAF::Processes::MissionTask"
    _BASE_METACLASSES = ("Activity",)
    _ABSTRACT = False
...
class MissionTaskAction(OperationalActivityAction, U.CallBehaviorAction):
    """The usage of a MissionTask within a MissionThread"""
    _STEREO = "UAF::Processes::MissionTaskAction"
    _BASE_METACLASSES = ("CallBehaviorAction",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("MissionTaskAction.behavior",
         "Value for activity metaproperty must be stereotyped «MissionTask» or its specializations"
         "."
        ),
        ("MissionTaskAction.activity",
         "Value for the activity metaproperty must be stereotyped «MissionThread» or its specializ"
         "ations."
        ),
    )

class MissionVignette(Condition, U.DataType):
    """Description of the geographical location and time frame of the overall conflict. A vignette includes information such as threat and friendly politico-military contexts and backgrounds, assumptions, constraints, limitations, strategic objectives, and other planning considerations. It can represent small, ideally self-contained parts of a scenario. (OUSD(R&E))"""
    _STEREO = "UAF::Parameters::MissionVignette"
    _BASE_METACLASSES = ("DataType",)
    _ABSTRACT = False
...
class OperationalRole(U.Property, AssetRole, MeasurableElement, LocationHolder, OpposableElement):
...
class Opportunity(MotivationalElement, U.Class, AffectableElement):
...
class Opposes(U.Abstraction, MeasurableElement):
    """A tuple that indicates an open clash between two opposing groups (or individuals). This means that elements in the model are fighting against each other or are providing strong resistance to progress towards some goals. Note to entry: By individual this does not indicate instances in the model."""
    _STEREO = "UAF::Information::Opposes"
    _BASE_METACLASSES = ("Abstraction",)
    _ABSTRACT = False
    CONSTRAINTS = (
        ("Opposes.client",
         "Values for the client metaproperty must be stereotyped by the specialization of «Opposab"
         "leElement»."
        ),
        ("Opposes.supplier",
         "Values for the supplier metaproperty must be stereotyped by the specialization of «Oppos"
         "ableElement»."
        ),
    )
...
class ResourceRole(U.Property, AssetRole, LocationHolder, SubjectOfResourceConstraint, MeasurableElement, OpposableElement):
...
# ---------------------------------------------------------------------------
_STEREOTYPES = [UAFElement, Achiever, MeasurableElement, Achieves, AffectableElement, PropertySet, Activity, ActualState, ActualPropertySet, ActualCondition, ActualEffect, ActualStrategicPhase, ActualEnduringTask, ActualEnterprisePhase, ActualEnvironment, ActualLocation, ActualMeasurement, ActualMeasurementSet, ActualMission, ActualMissionPhase, ActualMissionScenario, ActualMissionVignette, Stakeholder, SubjectOfResourceConstraint, CapableElement, ActualResource, ActualOrganizationalResource, ActualResponsibleResource, ActualOrganization, ActualResourceRole, ActualOrganizationRole, ActualOutcome, ActualPerson, ActualPost, ActualProject, ActualProjectMilestone, ActualProjectMilestoneRole, ActualProjectRole, ActualResourceRelationship, ActualResponsibility, ActualRisk, ActualService, Affects, AffectsInContext, Alias, ArbitraryConnector, ArchitecturalDescription, ArchitecturalReference, Architecture, Metadata, ArchitectureMetadata, ConceptItem, SubjectOfSecurityConstraint, Asset, AssetRole, Desirer, PhaseableElement, SubjectOfStrategicConstraint, Capability, ResourceAsset, Resource, ResourceExchangeItem, VersionedElement, SubjectOfForecast, OperationalExchangeItem, ServiceExchangeItem, StrategicExchangeItem, OpposableElement, ResourcePerformer, ResourceArchitecture, CapabilityConfiguration, CapabilityRole, MotivationalElement, Challenge, Exchange, ResourceExchange, Command, ComparesTo, Competence, CompetenceForRole, CompetenceToConduct, ConceptRole, Concern, Condition, ConflictsWith, Control, Creates, Defines, Definition, DesignationKind, Desires, Standard, Doctrine, Driver, Effect, Enables, SecurityControl, EnhancedSecurityControl, Enhances, EnterpriseGoal, EnterpriseObjective, EnterpriseVision, Environment, EnvironmentProperty, EvokedBy, Exhibits, FieldedCapability, FillsPost, Forecast, Function, FunctionAction, FunctionEdge, FunctionControlFlow, FunctionObjectFlow, GeoPoliticalExtentType, GovernedBy, HighLevelOperationalConcept, ISO8601DateTime, ImpactedBy, Implements, Information, SubjectOfOperationalConstraint, InformationModel, IsCapableToPerform, OperationalAsset, OperationalAgent, OperationalPerformer, KnownResource, Location, LocationHolder, MapsToCapability, MapsToGoal, Measurement, MeasurementSet, Sequence, MilestoneDependency, StrategicPhase, Mission, MissionEngineeringThread, MissionScenario, OperationalActivity, MissionThread, MissionTask, OperationalActivityAction, MissionTaskAction, MissionVignette, Mitigates, MotivatedBy, PhysicalResource, NaturalResource, OperationalActivityEdge, OperationalArchitecture, OperationalConnector, Rule, OperationalConstraint, OperationalControlFlow, OperationalExchange, OperationalInformation, OperationalInformationRole, OperationalInterface, OperationalMessage, OperationalMethod, OperationalMitigation, OperationalObjectFlow, OperationalParameter, OperationalPort, OperationalRole, OperationalSignal, OperationalSignalProperty, OperationalStateDescription, Opportunity, Opposes, OrganizationalResource, Organization, OrganizationInPhase, OwnsProcess, OwnsRisk, OwnsRiskInContext, OwnsValue, PerformsInContext, Person, Phases, Post, PresentedBy, ProblemDomain, Project, ProjectActivity, ProjectActivityAction, ProjectMilestone, ProjectMilestoneRole, ResourceRole, ProjectRole, ProjectSequence, ProjectStatus, ProjectTheme, Protects, ProtectsInContext, Protocol, ProtocolImplementation, ProtocolLayer, ProtocolStack, ProvidedServiceLevel, ProvidesCompetence, RequiredServiceLevel, RequiresCompetence, ResourceArtifact, ResourceConnector, ResourceConstraint, ResourceInformation, ResourceInformationRole, ResourceInterface, ResourceMessage, ResourceMethod, ResourceMitigation, ResourceParameter, ResourcePort, ResourceService, ResourceServiceInterface, ResourceSignal, ResourceSignalProperty, ResourceStateDescription, Responsibility, ResponsibleFor, Risk, SameAs, SecurityConstraint, SecurityControlFamily, SecurityEnclave, SecurityProcess, SecurityProcessAction, SecurityRisk, Service, ServiceArchitecture, ServiceConnector, ServiceContract, ServiceFunctionEdge, ServiceControlFlow, ServiceExchange, ServiceFunction, ServiceFunctionAction, ServiceInterface, ServiceMessage, ServiceMethod, ServiceObjectFlow, ServiceParameter, ServicePolicy, ServicePort, ServiceRole, ServiceSignal, ServiceSignalProperty, ServiceStateDescription, Software, StandardOperationalActivity, StatusIndicators, StrategicAsset, StrategicConstraint, StrategicExchange, StrategicInformation, StructuralPart, Supports, System, Technology, TemporalPart, ValueItem, ValueStream, VersionOfConfiguration, VersionSuccession, View, Viewpoint, VisionStatement, WholeLifeConfiguration, WholeLifeEnterprise]
_EXTENSIONS = {
    ...
    "ActualMission": (("InstanceSpecification", True),),
    "ActualMissionPhase": (("InstanceSpecification", True),),
    "ActualMissionScenario": (("InstanceSpecification", True),),
    "ActualMissionVignette": (("InstanceSpecification", True),),
    ...
    "ConflictsWith": (("Abstraction", True),),
    "Control": (("InformationFlow", True),),
    "Creates": (("Dependency", True),),
    "Defines": (("Element", True),),
    "Definition": (("Comment", True),),
    "DesignationKind": (("Element", True),),
    ...
    "Mission": (("Class", True),),
    "MissionEngineeringThread": (("Activity", True),),
    "MissionScenario": (("DataType", True),),
    "MissionTask": (("Activity", True),),
    "MissionTaskAction": (("CallBehaviorAction", True),),
    "MissionThread": (("Activity", True),),
    "MissionVignette": (("DataType", True),),
    ...
    "OpposableElement": (("Element", True),),
    "Opposes": (("Abstraction", True),),
